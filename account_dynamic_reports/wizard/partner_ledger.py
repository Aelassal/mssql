...
class InsPartnerLedger(models.TransientModel):
    ...
    def _partner_sub_group_domain(self):
        if 'vendor_sub_group_id' not in self.env['res.partner']._fields:
            return [('id', 'in', [])]
    ...
    def prepare_where(self, mode='strict'):
        ...
        # Exclude currency exchange revaluation entries (debit=0, credit=0, amount_currency!=0)
        where += " AND NOT (l.debit = 0 AND l.credit = 0 AND l.amount_currency != 0) "
    ...
    def prepare_main_lines(self):
        ...
        if self.partner_sub_group_ids:
            if 'vendor_sub_group_id' in self.env['res.partner']._fields:
                partner_company_domain += ['|', ('id', 'in', self.partner_sub_group_ids.ids), ('vendor_sub_group_id', 'in', self.partner_sub_group_ids.ids)]
            else:
                partner_company_domain.append(('id', 'in', self.partner_sub_group_ids.ids))
        ...
        # Unknown Partner (transactions with no partner assigned)
        if not self.partner_ids and not self.partner_sub_group_ids:
            unknown_result = {
                'id_list': [],
                'size': 0,
                'debit': 0,
            ...
            sql = ('''
                SELECT
                    COALESCE(SUM(CASE WHEN l.debit > 0 AND l.amount_currency > 0 THEN l.amount_currency ELSE 0 END), 0) AS debit_currency,
                    COALESCE(SUM(CASE WHEN l.credit > 0 AND l.amount_currency > 0 THEN l.amount_currency ELSE 0 END), 0) AS credit_currency,
                    array_agg(l.id) AS id_list,
                    COUNT(l.id) AS size,
                    COALESCE(SUM(l.debit),0) AS debit,
                    COALESCE(SUM(l.credit),0) AS credit,
                    COALESCE(SUM(l.debit - l.credit),0) AS balance,
                    COALESCE(SUM(l.amount_currency),0) AS balance_currency,
                    'unknown_partner' AS time_string
                ''' + self.prepare_from() + self.prepare_where(mode='strict') +
                   ''' AND l.partner_id IS NULL ''')
            cr.execute(sql)
            res = cr.dictfetchone() or {}
            unknown_result.update(res)
            unknown_result.update({
                'partner_id': False,
                'partner_name': _('Unknown Partner'),
                'currency_id': self.currency_id.id,
                'is_unknown_partner': True,
            })
            if unknown_result.get('size', 0) and unknown_result['size'] > 0:
                if self.display_accounts == 'with_transaction' and (
                        self.currency_id.is_zero(unknown_result.get('debit')) and self.currency_id.is_zero(unknown_result.get('credit'))
                ):
                    pass
                else:
                    pl_lines.append(unknown_result)

        # Attach annotations
        if pl_lines:
            line_refs = [str(l.get('partner_id')) for l in pl_lines if l.get('partner_id')]
            if 'report.line.annotation' in self.env:
                annotations = self.env['report.line.annotation'].get_annotations('partner_ledger', line_refs)
                for line in pl_lines:
                    ref = str(line.get('partner_id'))
                    anns = annotations.get(ref, [])
                    line['annotation'] = anns[0]['note'] if anns else ''
    ...
    def prepare_detailed_lines(self, move_line_ids=[], partner_id=False):
        ...
        partner_clause = 'AND l.partner_id IS NULL' if not partner_id else 'AND l.partner_id = %s' % partner_id
        # Initial
        ...
        # Indirectly-linked AMLs: lines with no partner reconciled against this partner's lines
        if partner_id:
            where_indirect = self.prepare_where(mode='strict')
            sql_indirect = ('''
                SELECT
                    'strict' AS ttype,
                    TRUE AS is_indirect,
                    ...
                    l.currency_id AS currency_id,
                    l.name AS lname,
                    CASE WHEN l.currency_id <> l.company_currency_id THEN l.amount_currency ELSE 0 END AS amount_currency,
                    CASE WHEN l.debit > 0 AND l.amount_currency > 0 THEN l.amount_currency ELSE 0 END AS debit_currency,
                    CASE WHEN l.credit > 0 AND l.amount_currency > 0 THEN l.amount_currency ELSE 0 END AS credit_currency,
                    l.debit AS debit,
                    l.credit AS credit,
                    l.balance AS balance,
                    CASE WHEN l.currency_id IS NOT NULL AND l.amount_currency IS NOT NULL THEN ABS(l.amount_currency) ELSE 0 END AS credit_currency
                ''' % lang_code + self.prepare_from() + '''
                    JOIN account_partial_reconcile partial ON (
                        partial.debit_move_id = l.id OR partial.credit_move_id = l.id
                    )
                    JOIN account_move_line aml_partner ON (
                        CASE WHEN partial.debit_move_id = l.id THEN partial.credit_move_id ELSE partial.debit_move_id END = aml_partner.id
                    )
                ''' + where_indirect + '''
                    AND l.partner_id IS NULL
                    AND aml_partner.partner_id = %s
                GROUP BY
                    l.date, l.id, m.id, l.matching_number,
                    l.date_maturity, j.code, a.name, l.currency_id, a.code_store, m.name, l.name, l.debit, l.credit, l.amount_currency
                ORDER BY l.date
                ''' % partner_id)
            cr.execute(sql_indirect)
            indirect_lines = cr.dictfetchall()
            # Insert indirect lines into list (they will appear after direct lines)
            final_list += indirect_lines
