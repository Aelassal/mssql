...
class InsTrialBalance(models.TransientModel):
    ...
    def prepare_where(self, mode='strict'):
        ...
        # Exclude currency exchange revaluation entries
    ...
    def _insert_hierarchy_lines(self, gl_lines):
        """Insert account group header rows with rollup totals for hierarchy display."""
        account_ids = [line['account_id'] for line in gl_lines if line.get('account_id')]
        accounts = self.env['account.account'].browse(account_ids)

        # Build group map: group -> list of account lines
        group_map = {}
        account_group_map = {}
        for account in accounts:
            group = account.group_id
            if group:
                account_group_map[account.id] = group
        ...
        for line in gl_lines:
            ...
            if account_id and account_id in account_group_map:
                group = account_group_map[account_id]
                # Walk up the hierarchy and insert group headers we haven't seen
                group_chain = []
                g = group
                while g:
                    if g.id not in seen_groups:
                        group_chain.append(g)
                        seen_groups.add(g.id)
                    g = g.parent_id
                # Insert in top-down order
                for g in reversed(group_chain):
                    result.append({
                        'account_id': None,
                        'account_name': g.name,
                        'account_code': g.code_prefix_start or '',
                        'currency_id': self.currency_id.id,
                        'is_group': True,
                        'group_level': self._get_group_level(g),
                        'id_list': [], 'size': 0,
                        'time_string': 'group_%s' % g.id,
                ...
                # Add indentation level to account line
                line['group_level'] = self._get_group_level(group) + 1
                line['is_group'] = False
            result.append(line)

        # Calculate group totals (sum children into parents)
        group_totals = {}
        for line in result:
            ...
            account_id = line.get('account_id')
            if account_id and account_id in account_group_map:
                g = account_group_map[account_id]
                while g:
                    if g.id not in group_totals:
                        group_totals[g.id] = {}
                    for key in ['debit', 'credit', 'balance', 'initial_debit', 'initial_credit',
                                'initial_balance', 'ending_debit', 'ending_credit', 'ending_balance',
                                'comparison_debit', 'comparison_credit', 'comparison_balance']:
                        group_totals[g.id][key] = group_totals[g.id].get(key, 0) + (line.get(key) or 0)
                    g = g.parent_id

        # Update group lines with totals
        for line in result:
            if line.get('is_group'):
                ts = line['time_string']
                group_id = int(ts.replace('group_', ''))
                if group_id in group_totals:
                    line.update(group_totals[group_id])
    ...
    def prepare_main_lines(self):
        ...
        # Insert account group hierarchy headers
        if self.show_hierarchy:
            gl_lines = self._insert_hierarchy_lines(gl_lines)
        ...
        for field in model_id.field_id:
            if field.name in vals.keys():
                ...
                if field.ttype == 'boolean':
                    update_dict.update({field.name: bool(vals[field.name])})
