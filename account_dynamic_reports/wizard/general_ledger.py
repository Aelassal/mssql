...
class InsGeneralLedger(models.TransientModel):
    ...
    def prepare_main_lines(self):
        ...
        # Attach annotations
        if gl_lines:
            line_refs = [str(l.get('account_id')) for l in gl_lines if l.get('account_id')]
            annotations = self.env['report.line.annotation'].get_annotations('general_ledger', line_refs)
            for line in gl_lines:
                ref = str(line.get('account_id'))
    ...
    def prepare_detailed_lines(self, move_line_ids=[], account_id=False):
        ...
        # Tax summary lines
        tax_lines = self._get_tax_summary_lines(account_id)
        for tl in tax_lines:
            final_list.append({
                'ttype': 'tax_summary',
                'lid': 'tax_%s_%s' % (account_id, (tl.get('tax_name') or 'unknown').replace(' ', '_')),
                'tax_name': tl.get('tax_name', ''),
                'base_amount': tl.get('base_amount', 0),
                'tax_amount': tl.get('tax_amount', 0),
            })

        return final_list

    def _get_tax_summary_lines(self, account_id):
        """Get tax base/amount summary for an account if it has tax-related entries."""
        cr = self.env.cr
        where = self.prepare_where(mode='strict')
        sql = """
            SELECT
                t.name ->> '%s' AS tax_name,
                SUM(CASE WHEN l.tax_line_id IS NOT NULL THEN l.balance ELSE 0 END) AS tax_amount,
                SUM(CASE WHEN l.tax_line_id IS NULL AND l.balance != 0 THEN l.balance ELSE 0 END) AS base_amount
            %s
            LEFT JOIN account_tax t ON (l.tax_line_id = t.id OR t.id = ANY(
                SELECT tax_id FROM account_move_line_account_tax_rel WHERE account_move_line_id = l.id
            ))
            %s
            AND l.account_id = %s
            AND t.id IS NOT NULL
            GROUP BY t.name
            HAVING SUM(ABS(l.balance)) > 0
        """ % (self.env.user.lang or 'en_US', self.prepare_from(), where, account_id)
        try:
            cr.execute("SAVEPOINT tax_summary")
            cr.execute(sql)
            result = cr.dictfetchall()
            cr.execute("RELEASE SAVEPOINT tax_summary")
            return result
        except Exception:
            cr.execute("ROLLBACK TO SAVEPOINT tax_summary")
            return []
