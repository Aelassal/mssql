...
class InsFinancialReport(models.TransientModel):
    ...
    def _compute_unallocated_earnings(self):
        """Compute current year unallocated earnings from income/expense accounts."""
        cr = self.env.cr
        where = self.prepare_where()

        # Get fiscal year start using company's compute_fiscalyear_dates if available
        try:
            fiscal_year_start = self.env.company.compute_fiscalyear_dates(self.date_to)
            fy_start = fiscal_year_start.get('date_from')
        except Exception:
            # Fallback: simple fiscal year start calculation
            fy_start = date(self.date_to.year, 1, 1)

        where += " AND l.date >= '%s' AND l.date <= '%s'" % (fy_start, self.date_to)

        sql = """
            SELECT COALESCE(SUM(l.balance), 0) as balance
            FROM account_move_line l
            JOIN account_move m ON (l.move_id=m.id)
            JOIN account_account a ON (l.account_id=a.id)
            LEFT JOIN res_currency c ON (l.currency_id=c.id)
            JOIN account_journal j ON (l.journal_id=j.id)
        """ + where + """
            AND a.account_type IN ('income', 'income_other', 'expense', 'expense_depreciation', 'expense_direct_cost')
        """
        cr.execute(sql)
        result = cr.dictfetchone()
        net_profit = -(result.get('balance', 0) or 0)  # Negate: income is credit (negative balance)

        if self.env.company.currency_id.is_zero(net_profit):
            return None

        return {
            'name': _('Current Year Unallocated Earnings'),
            'balance': net_profit,
            'comparison_balance': 0,
            'percentage_change': 0,
            'budget_balance': 0,
            'debit': 0,
            'credit': 0,
            'parent': 0,
            'self_id': 'unallocated_earnings',
            'type': 'report',
            'level': 2,
            'class': 'py-fin-main-tr py-tr-level-2 fst-italic',
            'company_currency_id': self.env.company.currency_id.id,
            'fin_report_type': 'sum',
            'display_detail': 'no_detail',
            'range_selection': 'from_the_beginning',
            'time_string': fields.Datetime.now().strftime("%H:%M:%S") + 'unallocated',
        }
    ...
    def _tag_horizontal_sides(self, lines):
        """Tag lines with 'left' or 'right' side for horizontal Balance Sheet layout.
        Assets go left, Liabilities and Equity go right."""
        if not lines:
            return lines
        current_side = 'left'
        for line in lines:
            ...
            if line.get('level', 0) <= 1:
                if any(kw in name_lower for kw in ['liability', 'liabilities', 'equity', 'capital']):
                    current_side = 'right'
                elif any(kw in name_lower for kw in ['asset']):
                    current_side = 'left'
            line['side'] = current_side
        return lines

    def _get_budget_record(self):
        """Get the budget record."""
        return self.budget_id if self.budget_id else False

    def _compute_budget_for_accounts(self, account_ids):
        """Get budget planned amounts for given account IDs from selected budget."""
        budget = self._get_budget_record()
        if not budget:
            return {}
        result = {}
        for line in budget.budget_line:
            budget_accounts = line.general_budget_id.account_ids
            for acc in budget_accounts:
                if acc.id in account_ids:
                    if line.date_from <= self.date_to and line.date_to >= self.date_from:
                        result[acc.id] = result.get(acc.id, 0) + line.planned_amount
        return result

    def _attach_budget_to_lines(self, lines):
        """Attach budget amounts to report lines."""
        if not self.budget_id:
            return
        # Collect all account IDs from lines
        all_account_ids = set()
        for line in lines:
            if line.get('type') == 'account' and line.get('account'):
                all_account_ids.add(line['account'])

        budget_map = self._compute_budget_for_accounts(all_account_ids)

        # Attach budget to account-level lines
        for line in lines:
            if line.get('type') == 'account' and line.get('account'):
                line['budget_balance'] = budget_map.get(line['account'], 0)
            else:
                line['budget_balance'] = 0

        # Roll up budget totals to report-level lines
        for line in lines:
            if line.get('type') == 'report':
                # Sum budget of child account lines
                self_id = line['self_id']
                children_budget = sum(
                    l.get('budget_balance', 0) for l in lines
                    if l.get('parent') == self_id and l.get('type') == 'account'
                )
                line['budget_balance'] = children_budget
    ...
    def get_account_lines(self):
        ...
        # Attach budget data if a budget is selected
        if self.budget_id:
            self._attach_budget_to_lines(lines)

        # Add Unallocated Earnings from P&L for Balance Sheet
        if self.account_report_id and self.date_to and 'balance' in (self.account_report_id.name or '').lower().replace(' ', ''):
            try:
                unallocated = self._compute_unallocated_earnings()
                if unallocated:
                    lines.append(unallocated)
            except Exception:
                pass  # Skip if fiscal year computation fails
        ...
        if self.hide_zero_balance == 'yes':
            ...
            for item in lines:
                if item['type'] in ['account', 'account_type']:
                    ...
                    new_lines.append(item)
            new_lines = self._tag_horizontal_sides(new_lines)
            return new_lines
        lines = self._tag_horizontal_sides(lines)
        return lines
    ...
    budget_id = fields.Many2one(
        ...
        help='Select a validated budget to compare against actual values')
    ...
    def prepare_values_for_component(self):
        ...
        view_format = {'choices': [], 'selectedValue': {'value': self.view_format}}
        ...
        for field in model_id.field_id:
            ...
            if field.name == 'view_format':
                view_format['choices'] = [{'value': a.value, 'label': a.name} for a in field.selection_ids]

        current_range_string = self.get_current_date_range_string()
        comparison_range_string = self.get_comparison_date_range_string()

        budget_choices = [{'value': b.id, 'label': b.name} for b in
            self.env['budget.budget'].search([('state', '=', 'validate')])]
        ...
        return {
            ...
            'view_format_selection': view_format,
            'budget_id': self.budget_id.id if self.budget_id else 0,
            'budget_name': self.budget_id.name if self.budget_id else '',
            'has_budget': bool(self.budget_id),
    ...
    def update_values_from_component(self, vals):
        update_dict = {}
        # Map view_format_selection back to view_format for the generic field loop
        if 'view_format_selection' in vals:
            vals['view_format'] = vals.pop('view_format_selection')
        # Map budget_selection back to budget_id for the generic field loop
        if 'budget_selection' in vals:
            budget_val = vals.pop('budget_selection')
            budget_selected = budget_val.get('selectedValue', {}).get('value', 0) if isinstance(budget_val, dict) else 0
            vals['budget_id'] = int(budget_selected) if budget_selected else False
        model_id = self.env['ir.model'].sudo().search([('model', '=', 'ins.financial.report')], limit=1)
        for field in model_id.field_id:
            if field.name in vals.keys():
                ...
                if field.ttype == 'many2one':
                    if field.name in vals and vals[field.name]:
                        update_dict.update({field.name: int(vals[field.name])})
                    elif field.name in vals:
                        update_dict.update({field.name: False})
                if field.ttype == 'date':
                    update_dict.update({field.name: vals[field.name]})
                if field.ttype == 'integer':
                    update_dict.update({field.name: int(vals[field.name]) if vals[field.name] else 0})
