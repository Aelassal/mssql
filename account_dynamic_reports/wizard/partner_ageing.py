...
class InsPartnerAgeing(models.TransientModel):
    ...
    aging_based_on = fields.Selection([
        ...
        ('date', 'Invoice Date')],
        string='Aging Based On', default='date_maturity')
    aging_interval = fields.Integer(
        string='Aging Interval (Days)', default=30,
        help='Number of days for each aging bucket')
    ...
    def prepare_bucket_list(self):
        ...
        # Use aging_interval to auto-compute buckets if set
        if self.aging_interval and self.aging_interval > 0:
            interval = self.aging_interval
            bucket_list = [interval * (i + 1) for i in range(5)]
        else:
    ...
    def prepare_main_lines(self):
        ...
        for partner in partner_ids:
            age_dict = {
                ...
                'partner_trust': partner.trust if hasattr(partner, 'trust') else False,
                'id_list': [],
                'size': 0,
                'currency_id': company_currency_id,
                'total': 0
            ...
            for period in period_dict:
                ...
                date_field = 'l.date' if self.aging_based_on == 'date' else 'COALESCE(l.date_maturity,l.date)'
                where = " AND l.partner_id = %s AND %s " % (partner.id, date_field)
        ...
        if account_id:
            ...
            for period, values in period_dict.items():
                ...
                if start and stop:
                    select += f"""
                        CASE
                            WHEN {date_field} BETWEEN '{stop}' AND '{start}'
                            THEN
                                SUM(l.balance) +
                                SUM(COALESCE(
                                    (SELECT SUM(amount) 
    ...
    def prepare_values_for_component(self):
        ...
        aging_based_on = {'choices': [], 'selectedValue': {'value': self.aging_based_on}}
        ...
        for field in model_id.field_id:
            ...
            if field.name == 'aging_based_on':
                aging_based_on['choices'] = [{'value': a.value, 'label': a.name} for a in field.selection_ids]
        return {
