...
export class PycusAnalyticReport extends Component {
    ...
    get filteredLines() {
        ...
        if (!query) return this.state.gl_lines;
        return this.state.gl_lines.filter(
            line => ((line.analytic_code || '') + ' ' + (line.analytic_name || '')).toLowerCase().includes(query)
        );
    }
    ...
    get sortedLines() {
        ...
        if (!this.state.sortField) return lines;
