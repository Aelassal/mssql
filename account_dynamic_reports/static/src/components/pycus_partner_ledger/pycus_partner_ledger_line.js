...
import { PycusAnnotation } from "../pycus_annotation/pycus_annotation";
...
export class PycusPartnerLedgerLine extends Component {
    setup(){
        ...
        onWillUpdateProps(async (nextProps) => {
            if (nextProps.unfoldAll !== this.props.unfoldAll) {
                const shouldUnfold = nextProps.unfoldAll;
                if (shouldUnfold !== this.state.subLinesVisibility) {
                    this.state.subLinesVisibility = shouldUnfold;
                    if (shouldUnfold) {
                        await this.fetchMoveLines();
                    }
                }
            }
        })
    ...
    openPartnerAction(ev, partner_id) {
        ev.preventDefault();
        ev.stopPropagation();
    ...
    isOverdue(date) {
        ...
        const maturity = luxon.DateTime.fromISO(date);
        return maturity < luxon.DateTime.now().startOf('day');
    }
}
PycusPartnerLedgerLine.components = { PycusAnnotation }
